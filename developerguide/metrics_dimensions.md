...
|  `PersistentMessage.Success`  |  The number of persistent messages successfully processed by the message broker, while the clients were disconnected from their persistent session within a time period that is determined by CloudWatch\.  | 
|  `PersistentMessage.Throttle`  |  The number of persistent messages throttled while the clients were disconnected from their persistent session within a time period that is determined by CloudWatch, because the clients exceeded the persistent message rate limit\.  | 
