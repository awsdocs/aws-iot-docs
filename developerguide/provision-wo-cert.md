...
## Provision a device by claim<a name="claim-based"></a>
...
1. Call [ `CreateProvisioningTemplate`](https://docs.aws.amazon.com/iot/latest/apireference/API_CreateProvisioningTemplate.html) to create a provisioning template\. This API returns a template ARN\. For more information, see [Fleet provisioning APIs](fleet-provision-api.md)\.
...
1. The device obtains a permanent certificate and private key by using one of these options\. The device will use the certificate and key for all future authentication with AWS IoT\.
   ...
   1. Call [ `CreateKeysAndCertificate`](https://docs.aws.amazon.com/iot/latest/apireference/API_CreateKeysAndCertificate.html) to create a new certificate and private key using the AWS certificate authority\.

      Or

   1. Call [ `CreateCertificateFromCsr`](https://docs.aws.amazon.com/iot/latest/apireference/API_CreateCertificateFromCsr.html) to generate a certificate from a certificate signing request that keeps its private key secure\.

1. From the device, call [https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html](https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html) to register the device with AWS IoT and create cloud resources\.
...
**Note**  
The device must perform the following steps such that it calls [https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html](https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html) within five minutes of connecting to AWS IoT with the temporary provisioning claim certificate\.
...
1. The device calls [https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html](https://docs.aws.amazon.com/iot/latest/apireference/API_RegisterThing.html) to register the device with AWS IoT and create cloud resources\. Remember that this must occur within five minutes of connecting to AWS IoT with the temporary provisioning claim certificate\.
